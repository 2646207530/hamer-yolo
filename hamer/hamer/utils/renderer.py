...
def custom_cam_crop_to_full(cam_bbox, box_center, box_size, img_size, fx, fy, cx, cy, depth_refine=None):
    """
    根据真实内参计算全图坐标系下的相机平移 T (Tx, Ty, Tz)
    depth_refine: 如果提供（Tensor [B] 或标量），将强制使用此深度
    """
    b = cam_bbox.shape[0]
    device = cam_bbox.device
    
    def to_tensor(val):
        if isinstance(val, (float, int)):
            return torch.full((b,), val, device=device).float()
        elif isinstance(val, torch.Tensor):
            if val.dim() == 0: return val.unsqueeze(0).repeat(b).float()
            if val.dim() == 1 and val.shape[0] == 1: return val.repeat(b).float()
            return val.float()
        return torch.tensor(val, device=device).float()

    fx = to_tensor(fx)
    fy = to_tensor(fy)
    cx_real = to_tensor(cx)
    cy_real = to_tensor(cy)
    
    # 1. 确定深度 Tz
    if depth_refine is not None:
        print("使用真实深度")
        tz = to_tensor(depth_refine)
        # 此时我们需要根据真实深度反推有效的 bs (即像素尺寸与焦距的关系)
        # 公式 tz = 2 * fx / bs  =>  bs = 2 * fx / tz
        bs = (2 * fx / (tz + 1e-9))
    else:
        print("使用预测深度")
        # 原始逻辑：通过模型预测的 scale (cam_bbox[:, 0]) 确定
        bs = box_size * cam_bbox[:, 0] + 1e-9
        tz = 2 * fx / bs

    # 2. 计算 Tx, Ty
    # 核心公式：基于相似三角形，像素偏移对应的物理位移 Tx = (delta_u * Tz) / fx
    # 在 HaMeR 模型中，位移由两部分组成：
    # Part 1: BBox 中心相对于主点的偏移
    # Part 2: cam_bbox[:, 1:] 提供的相对于 BBox 中心的微调偏移
    
    # 注意：HaMeR 的 cam_bbox[1] 是归一化的，它在投影空间对应的物理距离比例是 (2 / bs)
    tx = (2 * (box_center[:, 0] - cx_real) / bs) + cam_bbox[:, 1]
    ty = (2 * (box_center[:, 1] - cy_real) / bs) + cam_bbox[:, 2]
