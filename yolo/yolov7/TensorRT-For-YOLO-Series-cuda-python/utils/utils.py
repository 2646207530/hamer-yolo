...
class BaseEngine(object):
    ...
    # def get_fps(self, image_path): 
    #     import time
    #     if image_path:
    #         # 加载实际图像
    #         img = cv2.imread(image_path)
    #         img = cv2.resize(img, (self.imgsz[1], self.imgsz[0]))  # 调整为模型输入尺寸
    #         img = np.transpose(img, (2, 0, 1))  # 调整通道顺序为 (C, H, W)
    #         img = np.ascontiguousarray(img, dtype=np.float32)  # 确保内存连续
    #         img = np.expand_dims(img, axis=0)  # 增加批次维度 (1, 3, H, W)
    #     else:
    #         # 使用全白图像
    ...
    #     fps = 100 / total_time
    #     ms_per_frame = (total_time / 100) * 1000  # 每帧的毫秒数

    #     print(f"{fps} FPS")
    #     print(f"{ms_per_frame} ms per frame")
